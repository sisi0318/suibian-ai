...
class cURL
{
		...
		switch ($method) {
			...
			case 'proxy':
				if (isset($args[0]) && isset($args[1]))
					$this->options[CURLOPT_PROXY] = $args[0];
				$this->options[CURLOPT_PROXYPORT] = $args[1];
			...
			case 'sse':
			case 'stream':
				return $this->requestSSE($method, $args);
			default:
	...
	//SSE流式处理请求
	private function requestSSE($method, $args)
	{
		$callback = isset($args[0]) ? $args[0] : null;
		$onError = isset($args[1]) ? $args[1] : null;
		$onComplete = isset($args[2]) ? $args[2] : null;
		
		if (!is_callable($callback)) {
			throw new \InvalidArgumentException('SSE callback must be callable');
		}
		
		// 设置流式处理的选项
		$this->options[CURLOPT_WRITEFUNCTION] = function($ch, $data) use ($callback, $onError) {
			static $buffer = '';
			$buffer .= $data;
			...
			while (($pos = strpos($buffer, "\n\n")) !== false) {
				...
				// 解析 SSE 事件
				$event = $this->parseSSEEvent($chunk);
				if ($event) {
					try {
		...
		if (is_callable($onComplete)) {
			call_user_func($onComplete, $result, $info, $error);
		}
	...
	//解析SSE事件格式
	private function parseSSEEvent($chunk)
	{
		if (empty(trim($chunk))) {
			return null;
		}
